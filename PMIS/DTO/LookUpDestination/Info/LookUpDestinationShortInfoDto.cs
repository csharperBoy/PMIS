...
namespace PMIS.DTO.LookUpDestination.Info
{
    public class LookUpDestinationShortInfoDto
    {
        ...
        {
            LookUpDestinationShortInfoDto temp = GenericMapHandlerFactory.GetMapper(GenericMapHandlerFactory.MappingMode.Auto).Map<PMIS.Models.LookUpDestination, LookUpDestinationShortInfoDto>(baseModel, this);

            //this.FkLookUpInfo = (new LookUpSearchResponseDto()).extraMapFromBaseModel(baseModel.FkLookUp);
            temp.LookUpValuesInfo = baseModel.FkLookUp.LookUpValues.Select(v => (new LookUpValueTinyInfoDto(v)).extraMapFromBaseModel(v)).ToList();
